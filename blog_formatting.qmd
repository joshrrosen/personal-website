...
# Bayesian Hierarchical Modeling Part 2: Working with Stan
...
## Stan recap (i.e., what's the gist?)
...
# Add a single predictor, sort of
dat$pred <- rnorm(2408)

```

```{r, warning=FALSE, results='hide', eval=FALSE, message=FALSE}
...
# Extract the samples from the posterior distributions
#brms_samples1 <- as.data.frame(brms_fit, pars = "b_pred")
#stan_samples1 <- as.data.frame(stan_fit, pars = "beta[1]")

# Combine the samples into a single data frame
#combined_samples1 <- cbind(brms_samples1, stan_samples1)

#write.csv(combined_samples1, "C:\\Users\\User\\Desktop\\PhD_Folders\\personal-website\\combined_samples1.csv", row.names=FALSE)
combined_samples1 <- read.csv("C:\\Users\\User\\Desktop\\PhD_Folders\\personal-website\\combined_samples1.csv")

# Plot a histogram of each
combined_samples1 %>%
  ggplot() +
  geom_histogram(aes(x = b_pred, y = after_stat(density), fill = "brms coefficient"), bins = 60, alpha = 0.5, position = "identity") +
  geom_histogram(aes(x = `beta.1.`, y = after_stat(density), fill = "stan coefficient"), bins = 30, alpha = 0.5, position = "identity", adjust = 1.5, size = 1.5) +
  geom_vline(aes(xintercept = mean(b_pred)), col = "blue", linetype = "solid") +
  geom_vline(aes(xintercept = mean(`beta.1.`)), col = "red", linetype = "dashed") +
  scale_fill_manual(values = c("brms coefficient" = "blue", "stan coefficient" = "red")) +
  theme_minimal() +
  xlab("Coefficient Estimate") +
  labs(title = "Comparing Predictor Coefficients") +
  scale_x_continuous(breaks = seq(-10, 10, by = 1)) +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "Figure 1: Comparing Predictor Coefficients")


##### Intercept

# Extract the samples from the posterior distributions
#brms_samples2 <- as.data.frame(brms_fit, pars = "b_Intercept")
#stan_samples2 <- as.data.frame(stan_fit, pars = "alpha")

# Combine the samples into a single data frame
#combined_samples2 <- cbind(brms_samples2, stan_samples2)

#write.csv(combined_samples2, "C:\\Users\\User\\Desktop\\PhD_Folders\\personal-website\\combined_samples2.csv", row.names=FALSE)
combined_samples2 <- read.csv("C:\\Users\\User\\Desktop\\PhD_Folders\\personal-website\\combined_samples2.csv")

# Plot a histogram of each
combined_samples2 %>%
  ggplot() +
  geom_histogram(aes(x = b_Intercept, y = after_stat(density), fill = "brms coefficient"), bins = 60, alpha = 0.5, position = "identity") +
  geom_histogram(aes(x = `alpha`, y = after_stat(density), fill = "stan coefficient"), bins = 30, alpha = 0.5, position = "identity", adjust = 1.5, size = 1.5) +
  geom_vline(aes(xintercept = mean(b_Intercept)), col = "blue", linetype = "solid") +
  geom_vline(aes(xintercept = mean(`alpha`)), col = "red", linetype = "dashed") +
  scale_fill_manual(values = c("brms coefficient" = "blue", "stan coefficient" = "red")) +
  theme_minimal() +
  xlab("Intercept Estimate") +
  labs(title = "Comparing Intercept Estimate") +
  scale_x_continuous(breaks = seq(-10, 10, by = 1)) +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "Figure 2: Comparing Intercept Estimates")
...
[^2]: To be honest, I only somewhat answer this question
...
[^3]: The phrase "so you wouldn't have to" is really the theme of today's post. Nothing here is terribly surprising, but it's nice to see all plotted out for us.
...
#df <- read.csv("D:\\Downloads\\CRAIGSLIST.Listings.csv") %>%
#  mutate(annual = as.numeric(PRICE*12)) %>%
#  mutate(AREA_SQFT = as.numeric(AREA_SQFT)) %>%
#  #select(annual) %>%
#  na.omit() # remove NAs
...
# step 0: load your packages and download the data
library(tidyverse)
library(ggthemes)
library(stringr)

url <- "https://dataverse.harvard.edu/api/access/datafile/6316627"
...
[^4]: https://www.huduser.gov/portal/pdredge/pdr_edge_featd_article_092214.html
...
[^5]: https://www.census.gov/quickfacts/fact/table/MA/INC110221

[^6]: https://www.boston.gov/sites/default/files/file/2022/05/Presentation.pdf
...
[^7]: https://www.boston.gov/sites/default/files/file/2022/05/Presentation.pdf

```{r, message=FALSE, warning=FALSE, echo=FALSE, results='asis'}

# Plot histogram 
ggplot(df, aes(x = PRICE)) + 
  geom_histogram(colour = 1, fill = "grey", 
                 bins = 100) +
  labs(x = "Advertised Rent (Monthly)",
       title = "Distribution of Apartment Rent in Massachusetts",
       subtitle = "Craigslist Postings: 2020-2021",
       y = "Frequency") +
  theme_solarized() +
  theme(
...
[^8]: https://www.jchs.harvard.edu/americas-rental-housing-2022

## Replication

### Creating the Plot

I've included the code below, but there's nothing too complicated in how to organize the data for a presentation like what we see here. The one major thing to note here is that the code assumes your dataframe is titled **df**[^9], and annual rental costs are organized in a column called **annual**.
...
[^12]: verifying this goes a bit beyond the scope of this blog, but we can use Bayes rule to express the full conditional distribution into probability density functions: $p(\alpha_j | \mathbf{y}, \mu_{\alpha}, \sigma_y, \sigma_{\alpha}) \propto p(\mathbf{y} | \alpha_j, \sigma_y) \times p(\alpha_j | \mu_{\alpha}, \sigma_{\alpha})$.
...
[^13]: https://www.jstatsoft.org/article/view/v080i01, https://www.jmlr.org/papers/volume15/hoffman14a/hoffman14a.pdf, https://www.sciencedirect.com/science/article/pii/037026938791197X?via%3Dihub
...
[^14]: https://mc-stan.org/rstan/reference/Rhat.html
...
[^15]: The details for both effective sample size and $\hat{R}$ are not important for the scope of this tutorial.
