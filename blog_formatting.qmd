...
[^1]: To be honest, I only somewhat answer this question
...
[^2]: The phrase "so you wouldn't have to" is really the theme of today's post. Nothing here is terribly surprising, but it's nice to see all plotted out for us.
...
[^4]: https://www.census.gov/quickfacts/fact/table/MA/INC110221
...
[^7]: https://www.jchs.harvard.edu/americas-rental-housing-2022

## Replication

### Creating the Plot

I've included the code below, but there's nothing too complicated in how to organize the data for a presentation like what we see here. The one major thing to note here is that the code assumes your dataframe is titled **df**[^8], and annual rental costs are organized in a column called **annual**.
...
[^11]: verifying this goes a bit beyond the scope of this blog, but we can use Bayes rule to express the full conditional distribution into probability density functions: $p(\alpha_j | \mathbf{y}, \mu_{\alpha}, \sigma_y, \sigma_{\alpha}) \propto p(\mathbf{y} | \alpha_j, \sigma_y) \times p(\alpha_j | \mu_{\alpha}, \sigma_{\alpha})$.
...
[^12]: https://www.jstatsoft.org/article/view/v080i01, https://www.jmlr.org/papers/volume15/hoffman14a/hoffman14a.pdf, https://www.sciencedirect.com/science/article/pii/037026938791197X?via%3Dihub
...
[^13]: https://mc-stan.org/rstan/reference/Rhat.html
...
[^14]: The details for both effective sample size and $\hat{R}$ are not important for the scope of this tutorial.
